...
class App:  
    ...
    def programBeam(self):
        ...
        if self.res_bits.get()==1:
            PhaseValues = np.arange(-220, 220, phase_step_size)
        if self.sweep.get() == 1:
        ...
        for PhDelta in PhaseValues:
            ...
            value1 = (self.c * np.radians(np.abs(PhDelta)))/(2*3.14159*(self.SignalFreq-bandwidth*1000000)*self.d)
            ...
            phase_delta.append(PeakValue_angle)
            angle.append(SteerAngle)
            if np.sign(PeakValue_angle)==-1:
                target_error = min(-0.01, (np.sign(PeakValue_angle) * (PeakValue_sum - PeakValue_delta) + np.sign(PeakValue_angle) * (PeakValue_sum + PeakValue_delta)/2) / (PeakValue_sum + PeakValue_delta))
            else:
                target_error = max(0.01, (np.sign(PeakValue_angle) * (PeakValue_sum - PeakValue_delta) + np.sign(PeakValue_angle) * (PeakValue_sum + PeakValue_delta)/2) / (PeakValue_sum + PeakValue_delta))
            diff_error.append(target_error)

        self.ArrayGain = gain
        self.ArrayDelta = delta
        self.ArrayPhaseDelta = phase_delta
    ...
    def plotData(self):
        ...
        self.ax1.cla()
        self.ax1.plot(self.ArrayAngle, self.ArrayGain, '-o', ms=3, alpha=0.7, mfc='blue')
        self.ax1.plot(self.ArrayAngle, self.ArrayDelta, '-o', ms=3, alpha=0.7, mfc='red')
        self.ax1.plot(self.saved_angle, self.saved_gain, '-o', ms=1, alpha=0.5, mfc='green')
        ...
        self.ax2.cla()
        self.ax2.plot(self.ArrayAngle, np.sign(self.ArrayPhaseDelta), '-o', ms=3, alpha=0.7, mfc='blue')
        self.ax2.plot(self.ArrayAngle, self.ArrayError, '-o', ms=3, alpha=0.7, mfc='red')
        ...
        self.pol_ax1.plot(np.radians(self.saved_angle), self.saved_gain, '-o', ms=1, alpha=0.5, mfc='green')
        ...
        self.ax4.cla()
        self.ax4.plot(self.ArrayAngle, np.sign(self.ArrayPhaseDelta), '-o', ms=3, alpha=0.7, mfc='blue')
        self.ax4.set_xlabel('Steering Angle (deg)')
        self.ax4.set_ylabel('time (s)')
        self.ax4.set_xlim([x_axis_min, x_axis_max])
