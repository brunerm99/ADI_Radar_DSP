...
beam0.reset()                       #Performs a soft reset of the device (writes 0x81 to reg 0x00)

beam0._ctrl.reg_write(0x00, 0x18)    #Sets the SDO pin to active, i.e. a 4 wire SPI bus.  Before each reg_write, 0x18 is also written to 0x00.
beam0._ctrl.reg_write(0x400, 0x55)   #This trims the LDO value to approx. 1.8V (to the center of its range)
beam0._ctrl.reg_write(0x38, 0x60)    #Bypasses the beam and bias RAM, i.e. we use the SPI writes to set the Gain, Phase and bias values instead of pulling the values the memory
beam0._ctrl.reg_write(0x34, 0x08)    #Sets the LNA bias to the middle of its range
beam0._ctrl.reg_write(0x35, 0x16)    #Sets the VGA bias to [0010] and the vector modulator bias to [110]
beam0._ctrl.reg_write(0x2E, 0x07)    #Enables the Rx LNA, Rx Vector Modulator and Rx VGA
beam0._ctrl.reg_write(0x31, 0x08)    #Enables switch driver for ADTR1107 switch
...
if DEVICE_MODE == "rx":
    ...
    beam0.mode = "rx"   # enables bit 7 and 5 of reg 0x31
    #beam0._ctrl.reg_write(0x31, 0xB0)   #Enables the whole Rx and sets ADTR1107 switch high (Rx mode)
