...
#config.Tx_freq    = 5.9e9          # Tx LO freq.  This is the freq that Pluto generates (if using the Tx of Pluto as a crude LO)
config.LO_freq    = 12e9       # Frequency to program the ADF4159's VCO to.  It is the external LO of the LTC555x mixers
config.SignalFreq = 10.106e9
#config.SignalFreq = config.LO_freq - config.Tx_freq
#config.Rx_freq    = config.SignalFreq - config.Tx_freq
config.Rx_freq    = config.LO_freq - config.SignalFreq
#config.Rx_freq = config.Tx_freq
config.Tx_freq = config.Rx_freq
...
config.RxGain7 = 127
config.RxGain8 = 127
config.Rx1_cal = 0  # you can put phase cal values here (to compensate for phase mismatches in the lines, etc.)
config.Rx2_cal = 0
config.Rx3_cal = 0
config.Rx4_cal = 0
config.Rx5_cal = 0
...
config.d = 0.015          # element to element spacing of the antenna
