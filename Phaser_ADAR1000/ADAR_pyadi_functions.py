...
def ADAR_set_mode(beam, mode):
    if mode == "rx":
        ...
        print(beam._ctrl.reg_read(0x031))
...
def ADAR_set_Phase(beam, mode, PhDelta, phase0, phase1, phase2, phase3):
    for channel in beam.channels:
        if mode == "rx":
            beam.channel1.rx_phase=phase0 + PhDelta*0
            beam.channel2.rx_phase=phase1 + PhDelta*1
            beam.channel3.rx_phase=phase2 + PhDelta*2
            beam.channel4.rx_phase=phase3 + PhDelta*3
            beam.latch_rx_settings()
        else:
            beam.latch_tx_settings()
