...
class App:  
    def __init__(self, master):      
        ...
        self.Rx_freq   = config.Rx_freq+1e6           # Rx LO freq of Pluto/ADRV9361
        self.LO_freq  = config.LO_freq       #freq of the LO going to the LTC5553 mixers
        ...
        self.sdr = SDR_init(self.sdr_address, self.num_Rx, self.SampleRate, self.Tx_freq, self.Rx_freq, self.Rx_gain, -3)
        SDR_LO_init(self.LO_freq)
        self.beam0 = adi.adar1000(
            chip_id="BEAM0",
            array_element_map=[[1, 2, 3, 4]],
            channel_element_map=[2, 1, 4, 3])
        ADAR_init(self.beam0)
        ADAR_set_mode(self.beam0, "rx")
        if self.num_ADARs==2:
    ...
    def programTaper(self):
        ...
        ADAR_set_Taper(self.beam0, "rx", self.RxGain1, self.RxGain2, self.RxGain3, self.RxGain4)
        if self.num_ADARs == 2:
    ...
    def programLO(self):
        ...
        #self.Rx_freq = int(self.SignalFreq - self.Mixer_LO)
        self.Rx_gain = int(self.RxGain.get())
        SDR_setRx(self.sdr, self.num_Rx, self.Rx_freq, self.Rx_gain)
    ...
    def programBeam(self):
        ...
        for PhDelta in PhaseValues:   
            ADAR_set_Phase(self.beam0, 'rx', PhDelta, self.RxPhase1, self.RxPhase2, self.RxPhase3, self.RxPhase4)
            if self.num_ADARs == 2:
                ADAR_set_Phase(spi, self.ADDR2, 2, PhDelta, phase_step_size, self.RxPhase5, self.RxPhase6, self.RxPhase7, self.RxPhase8)
                ADAR_update_Rx(spi, self.ADDR2)
            SteerAngle = self.ConvertPhaseToSteerAngle(PhDelta)
            total = 0
            for count in range (0, self.Averages):
                data_raw=SDR_getData(self.sdr)
                data = data_raw
